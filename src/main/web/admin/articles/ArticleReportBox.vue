<template>
<div class="box">
  <div class="box-header">
    <h3 class="box-title">文章举报</h3>

    <div class="box-tools">
      <div class="input-group input-group-sm" style="width: 150px;">
        <input type="text" class="form-control pull-right" placeholder="搜索" v-model="keywordFromInput">
        <div class="input-group-btn">
          <button type="button" class="btn btn-default" @click="search"><i class="fa fa-search"></i></button>
        </div>
      </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body table-responsive no-padding">
    <table class="table table-hover">
      <thead>
        <tr>
          <th>#</th>
          <th>举报人</th>
          <th>举报信息</th>
          <th>文章ID</th>
          <th>文章预览</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
        <article-report-list-row v-for="report in reports" :key="report.id" :report="report"
          @deleteArticle="deleteArticle" @recoverArticle="recoverArticle" @expandContent="expandContent"
          @closeReport="closeReport" @openReport="openReport" />
      </tbody>
    </table>
  </div>
  <!-- /.box-body -->

  <div v-if="pagination.totalPages > 1" class="box-footer clearfix">
    <pagination :pagination="pagination" @changePage="fetchArticleReports" />
  </div>
</div>
</template>

<script>
import ArticleReportListRow from './ArticleReportListRow.vue'
import Pagination from '../Pagination.vue'


export default {
  name: 'ArticleBox',
  components: {
    ArticleReportListRow,
    Pagination
  },
  data() {
    return {
      reports: [{
        id: 1,
        profile: {
          nickname: '小蛤蛤',
          id: 3
        },
        reason: '吓死我了这个文章',
        itemId: 4,
        itemType: 1,
        closed: false
      }],
      pagination: {
        currentPage: 0,
        totalPages: 1
      },
      pageSize: 2,
      keywordFromInput: '',
      keywordOfResult: ''
    }
  },
  created() {
    this.fetchArticleReports(0)
  },
  methods: {
    fetchArticleReports(page) {
      let url = ''
      if (this.keywordOfResult) {
        url = '/api/admin/reports/articles?' + 'page=' + page + '&size=' + this.pageSize + '&keyword=' + this.keywordOfResult
      } else {
        url = '/api/admin/reports/articles?' + 'page=' + page + '&size=' + this.pageSize
      }

      this.$http.get(url)
        .then(function(response) {
          this.reports = response.body.content;
          this.pagination.currentPage = response.body.number;
          this.pagination.totalPages = response.body.totalPages;
        }, function(response) {
          alert(response.body.message || '获取文章举报失败');
        })
    },
    closeReport(report) {
      // TODO
      alert('还没实现呢')
    },
    openReport(report) {
      // TODO
      alert('还没实现呢')
    },
    deleteArticle(article) {
      this.$emit('deleteArticle', article)
    },
    recoverArticle(article) {
      this.$emit('recoverArticle', article)
    },
    expandContent(article) {
      this.$emit('expandContent', article)
    },
    search() {
      this.keywordOfResult = this.keywordFromInput;
      this.fetchArticleReports(0);
    }
  }
}
</script>

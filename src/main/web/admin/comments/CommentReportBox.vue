...
<div class="box">
  ...
  <div class="box-body table-responsive no-padding">
    <table class="table table-hover">
      ...
      <tbody>
        <comment-report-list-row v-for="report in reports" :key="report.id" :report="report"
  ...
  <div v-if="pagination.totalPages > 1" class="box-footer clearfix">
    <pagination :pagination="pagination" @changePage="fetchCommentReports" />
  </div>
</div>
</template>

<script>
import CommentReportListRow from './CommentReportListRow.vue'
...
export default {
  ...
  created() {
    this.fetchCommentReports(0)
  },
  methods: {
    fetchCommentReports(page) {
      let url = ''
      if (this.keywordOfResult) {
        url = '/api/admin/reports/comments?' + 'page=' + page + '&size=' + this.pageSize + '&keyword=' + this.keywordOfResult
      } else {
        url = '/api/admin/reports/comments?' + 'page=' + page + '&size=' + this.pageSize
      }
      ...
      this.$http.get(url)
        .then(function(response) {
          ...
          alert(response.body.message || '获取评论举报失败');
        })
    },
    closeReport(report) {

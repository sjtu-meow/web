...
public class UserServiceImpl implements UserService {
    ...
    public Iterable<User> findAll(String keyword, boolean isAdmin) {
        return isAdmin
                ? userRepository.findByKeyword(StringUtil.tryStringToPosLong(keyword),
                        StringUtil.wrapLikeSubstr(keyword))
                : userRepository.findByKeywordActive(StringUtil.tryStringToPosLong(keyword),
                        StringUtil.wrapLikeSubstr(keyword));
    }
    ...
    public Iterable<User> findAllPageable(Integer page, Integer size, String keyword, boolean isAdmin) {
        return isAdmin
                ? userRepository.findByKeyword(StringUtil.tryStringToPosLong(keyword),
                        StringUtil.wrapLikeSubstr(keyword), new PageRequest(page, size))
                : userRepository.findByKeywordActive(StringUtil.tryStringToPosLong(keyword),
                        StringUtil.wrapLikeSubstr(keyword), new PageRequest(page, size));
    }
